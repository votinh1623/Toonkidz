...
import { getPosts, likePost, addComment, editComment, deleteComment, deletePostApi } from "../../service/postService";
...
const Discover = () => {
  ...
  const handleNavigateToProfile = (authorId) => {
    if (currentUser && currentUser._id === authorId) navigate('/home/profile');
    else navigate(`/home/profile/${authorId}`);
  };
  ...
  const handleSendOrUpdateComment = async (postId) => {
    ...
    if (isEditing) handleUpdateComment(postId);
    else handleAddNewComment(postId);
  };
  ...
  const handleAddNewComment = async (postId) => {
    ...
    try {
      ...
      } else message.error(res.error || "Gửi bình luận thất bại.");
    } catch (error) { message.error("Đã xảy ra lỗi khi gửi bình luận."); }
  };
  ...
  const handleUpdateComment = async (postId) => {
    ...
    try {
      ...
      } else message.error(res.error || "Cập nhật thất bại.");
    } catch (error) { message.error("Đã xảy ra lỗi khi cập nhật."); }
  };
  ...
  const handleInputChange = (storyId, value) => { setInputs((prev) => ({ ...prev, [storyId]: { ...(prev[storyId] || { text: "", rating: 0 }), text: value } })); };
  const handleSetRating = (storyId, rating) => { setInputs((prev) => ({ ...prev, [storyId]: { ...(prev[storyId] || { text: "", rating: 0 }), rating } })); };
  ...
  const renderCommentMenu = (comment, post) => {
    ...
    return (<Menu><Menu.Item key="report" onClick={() => handleReport(comment._id, 'Comment', comment.userId.name)}>Báo cáo bình luận này</Menu.Item></Menu>);
  };
  ...
  const renderPostMenu = (post) => {
    ...
    return (<Menu><Menu.Item key="report" onClick={() => handleReport(post._id, 'Post', post.userId.name)}>Báo cáo bài viết</Menu.Item></Menu>);
  };
  ...
  if (loading) return <div className="discover-loading"><Spin size="large" /></div>;
  ...
  return (
    <>
      <div className="discover-container">
        ...
        <div className="story-list">
          {posts.length > 0 ? posts.map((post) => {
            ...
            if (post.userId.isActive === false) return <div key={post._id} className="story-card"><ContentUnavailable /></div>;
            const isSharedPost = post.postType === 'share';
            ...
            <div className="no-posts-found"><h3>Chưa có bài đăng nào</h3></div>
          )}
        </div>
      </div>
      <StoryDetailModal
      ...
      <ShareOptionsModal
        ...
        post={postToShare}
        onShareToProfile={handleShareToProfile} o
        nShareToChat={handleShareToChat}
      />
      <ShareToProfileModal
        open={shareProfileOpen}
        onClose={() => setShareProfileOpen(false)}
        post={postToShare}
        onShared={(newPost) => { message.success("Đã chia sẻ!"); setShareProfileOpen(false); }}
      />
      <ShareToChatModal
        open={shareChatOpen}
